...
import { CallPerformance } from "../types";
import { getCallStatusColor, STATUS_COLORS } from "../utils";
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        <div className="p-3 bg-blue-50 rounded-lg border border-blue-200 text-center">
          ...
          <p className="text-xl font-bold text-blue-700">
            {callPerformance.total_calls.toLocaleString()}
          </p>
        </div>

        <div className="p-3 bg-green-50 rounded-lg border border-green-200 text-center">
          <p className="text-xs text-gray-500 font-medium uppercase">Connected</p>
          <p className="text-xl font-bold text-green-700">
            {callPerformance.connected_calls.toLocaleString()}
          </p>
        </div>

        <div className="p-3 bg-purple-50 rounded-lg border border-purple-200 text-center">
          <p className="text-xs text-gray-500 font-medium uppercase">Connection Rate</p>
          <p className="text-xl font-bold text-purple-700">
            {callPerformance.connection_rate.toFixed(1)}%
          </p>
        </div>

        <div className="p-3 bg-orange-50 rounded-lg border border-orange-200 text-center">
          <p className="text-xs text-gray-500 font-medium uppercase">No Answer</p>
          <p className="text-xl font-bold text-orange-700">
            {callPerformance.no_answer_calls.toLocaleString()}
          </p>
        </div>
      </div>
