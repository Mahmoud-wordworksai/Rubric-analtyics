...
import { Modal, Button, Typography, Alert, List, Tag, Switch } from 'antd';
import { FileText, Download, Loader2, CheckCircle, XCircle, Clock, Trash2 } from 'lucide-react';
import { startGroupExport, getExportJobStatus, downloadExportFile, type ExportJobStatus } from '../services/api';
import { useRoomAPI } from '@/hooks/useRoomAPI';

const { Text, Title } = Typography;

const STORAGE_KEY = 'call_reports_export_jobs';
...
const CallReportsModal: React.FC<CallReportsModalProps> = ({
  visible,
  groupId,
  datasheetName,
  onClose
  ...
  const [misEnabled, setMisEnabled] = useState(false);
  ...
  const pollStatus = useCallback(async (jobId: string) => {
    try {
      const status: ExportJobStatus = await getExportJobStatus(jobId, selectedRoom);
  ...
  const handleStartExport = async () => {
    ...
    try {
      ...
      const result = await startGroupExport(groupId, selectedRoom, misEnabled);
  ...
  const handleDownload = async (job: StoredJob) => {
    try {
      ...
      await downloadExportFile(job.jobId, job.filename || `call_report_${job.datasheetName}.csv`, selectedRoom);
        ...
        <div className="border-b pb-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <Text type="secondary" className="text-sm">MIS Export</Text>
                <Switch
                  checked={misEnabled}
                  onChange={setMisEnabled}
                  disabled={hasProcessingJob || isStarting}
                  size="small"
                />
              </div>
              <Button
